...
const mockReviews = [
  {
    ...
    contents: [
      ...
      },
      {
        ...
        date: "2026-04-28",
        type: "텍스트",
        prompt: "강남 아파트 매매 블로그 글",
        result: "강남에서 아파트 매매를 고민하고 계신가요?",
        status: "검수대기",
      },
      {
        id: 103,
  ...
  },
  {
    ...
    contents: [
      ...
      },
      {
        ...
        result: "체계적인 커리큘럼으로 성적 향상을 보장합니다.",
        status: "검수대기",
      },
    ],
  },
...
export default function ReviewPage() {
  ...
  const [generatingImage, setGeneratingImage] = useState(null); // contentId

  const toggleOpen = (id) => setOpenId((prev) => (prev === id ? null : id));

  const updateStatus = async (clientId, contentId, status) => {
    // UI 먼저 업데이트
    setClients((prev) =>
      prev.map((c) =>
        c.id === clientId
    ...
    // 시트에서 해당 콘텐츠 rowIndex 찾아서 업데이트
    try {
      const client = clients.find((c) => c.id === clientId);
      const content = client?.contents.find((item) => item.id === contentId);
      if (!content?.rowIndex) return;

      await fetch("/api/contents", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ rowIndex: content.rowIndex, status }),
      });
    } catch (error) {
  ...
  const approveAll = async (clientId) => {
    const client = clients.find((c) => c.id === clientId);
    if (!client) return;
    ...
    try {
      ...
      console.error("전체 승인 오류:", error);
    }
  };

  const handleGenerateImage = async (clientId, contentId) => {
    const client = clients.find((c) => c.id === clientId);
    if (!client) return;
  ...
  const getClientStatus = (contents) => {
    ...
    return {
      label: "대기 " + (total - approved) + "건",
      style: styles.badgeWaiting,
    };
  };
                    ...
                    {client.contents.map((item) => (
                      <div key={item.id} className={styles.contentItem}>
                        <div className={styles.contentHeader}>
                          ...
                          <span
                            className={`${styles.badge} ${item.status === "승인완료" ? styles.badgeApproved : item.status === "반려" ? styles.badgeRejected : styles.badgeWaiting}`}
                          >
                            {item.status}
                          </span>
                        </div>
                        ...
                        {item.type === "텍스트" ? (
                          ...
                          <div className={styles.imageWrapper}>
                            ...
                            <button
                              onClick={() =>
                                handleGenerateImage(client.id, item.id)
                              }
